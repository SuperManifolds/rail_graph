...
pub fn detect_line_conflicts(
...

    {
        ...
        log::debug!("Setup time: {setup_time:?}");
    }
...

    {
        ...
        log::debug!("Total detection time: {total_time:?}");
        log::debug!("Found {} conflicts, {} crossings from {} journeys", results.conflicts.len(), results.station_crossings.len(), train_journeys.len());

        // Print detailed timing breakdown
        log::debug!("=== Detailed Timing Breakdown ===");
        ...
        {
            log::debug!("Platform checks:     {:>10.3}ms", platform_ns as f64 / 1_000_000.0);
            log::debug!("  Extract occupancy: {:>10.3}ms", platform_extract_ns as f64 / 1_000_000.0);
            log::debug!("  Compare occupancy: {:>10.3}ms", platform_compare_ns as f64 / 1_000_000.0);
            log::debug!("Segment checks:      {:>10.3}ms", segment_ns as f64 / 1_000_000.0);
            log::debug!("  Segment pairs:     {segment_pair_calls:>10} calls");
            log::debug!("  HashMap lookups:   {:>10.3}ms", lookup_ns as f64 / 1_000_000.0);
            log::debug!("  Intersections:     {:>10.3}ms", intersection_ns as f64 / 1_000_000.0);
        }
        log::debug!("=================================");
    }
...
fn detect_conflicts_sweep_line(
    ...
    log::debug!("Using sweep-line algorithm ({} journeys)", train_journeys.len());
...

    {
        ...
        log::debug!("Segment map & platform cache build time: {cache_time:?}");
    }
...

    {
        ...
        log::debug!("Comparison loop time: {comparison_time:?}");
        let n = train_journeys.len();
        let naive_comparisons = n.saturating_mul(n.saturating_sub(1)) / 2;
        log::debug!("Made {comparisons} comparisons (vs {naive_comparisons} for naive O(n²))");
    }
}
