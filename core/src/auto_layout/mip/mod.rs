...
fn merge_station_clusters(
    ...
    if merged > 0 {
        log::info!("MIP layout: merged {merged} cluster stations");
    }
}
...
pub fn run_mip_layout(
    ...
    log::info!("MIP layout: {} hub spacing pairs", spacing_pairs.len());
    ...
    if !solution.success {
        log::info!("MIP layout: solver failed");
        return None;
    }
    ...
    for chain in &collapsed.chains {
        ...
        if parallel_pairs.contains(&pair) {
            ...
            log::info!("  Parallel: {a} <-> {b} ({} intermediates)", chain.intermediates.len());
        }
    }
    ...
    log::info!("MIP layout: {} parallel local/express pairs", parallel_pairs.len());
