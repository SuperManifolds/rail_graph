...
fn get_all_shortcut_definitions(is_mac: bool) -> Vec<ShortcutEntry> {
...
impl KeyboardShortcuts {
    /// Get the default keyboard shortcuts for the given platform
    #[must_use]
    pub fn default_shortcuts_for_platform(is_mac: bool) -> Self {
        let shortcuts = get_all_shortcut_definitions(is_mac)
            .into_iter()
    ...
    #[must_use]
    pub fn get_all_metadata(is_mac: bool) -> HashMap<String, ShortcutMetadata> {
        get_all_shortcut_definitions(is_mac)
            .into_iter()
            .map(|entry| {
                (
                    entry.id.to_string(),
    ...
    #[must_use]
    pub fn get_all_ordered(is_mac: bool) -> HashMap<ShortcutCategory, Vec<(String, ShortcutMetadata)>> {
        let mut grouped: HashMap<ShortcutCategory, Vec<(String, ShortcutMetadata)>> = HashMap::new();

        for entry in get_all_shortcut_definitions(is_mac) {
    ...
    pub fn merge_with_defaults(&mut self, is_mac: bool) {
        let defaults = Self::default_shortcuts_for_platform(is_mac);
...
impl Default for KeyboardShortcuts {
    fn default() -> Self {
        Self::default_shortcuts_for_platform(false)
    }
}
